...
export default () => {
  ...
  const fetching = async (semestr) => {
    ...
    try {
      ...
      const res = await (
        await fetch(
          `http://schedule.kpi.kharkov.ua/JSON/kabinet?email=${userEmail}&pass=${userPassword}&page=2&semestr=${semestr}`
        )
      ).json();
      setSubjRes(
        res.map((el) => ({
